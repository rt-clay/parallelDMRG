...
splitWavefunction(Environment const& env,
    ...
    if(env.firstNode()) 
        ...
        if(args.defined("BoundarySize"))
            {
            P = Partition(psi.length(),Nnode,args.getInt("BoundarySize"));
            }
        else
            {
            P = Partition(psi.length(),Nnode);
            }
        println(P);
...
template<typename ITensor, typename HType = ITensor>
struct Boundary
    {
    HType HH;
    ITensor A;
...
pdmrgWorker(Environment const& env,
    ...
    for(int sw = 1; sw <= sweeps.nsweep(); ++sw)
        ...
        printfln("Doing sweep %d for node %d (maxDim=%d, cutoff=%.0E, minDim=%d)",
        ...
        for(psw.newSweep(); psw.doingFull(); ++psw)
            ...
            if(psw.atRight() && dir==Fromleft && bool(mboxR))
                ...
                Boundary<ITensor,EdgeType> B;
                mboxR.receive(B);
                psi.Aref(n+1) = B.A;
                PH.R(B.HH);
                B = Boundary<ITensor,EdgeType>(); //to save memory
